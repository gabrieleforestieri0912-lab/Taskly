              ...
              <div className="p-6 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-3xl bg-gray-50/50 dark:bg-gray-900/10 flex flex-col items-center gap-4 text-center">
                ...
                <div className="flex gap-2 w-full max-w-md">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        const embed = getYouTubeEmbedUrl(e.target.value);
...
export default function NotesView({
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      setData({ blocks: finalBlocks, tags });
      setIsSaving(true);
      setTimeout(() => setIsSaving(false), 1000);
    }, 800);
    return () => clearTimeout(timeout);
  }, [blocks, setData, tags, isNoteLocked, isCryptedActive, passcode]);
