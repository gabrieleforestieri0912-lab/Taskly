...
export default function CalendarView({
  ...
  const getTasksForDay = (day) => {
    ...
    return tasksList.filter((t) => t.date === dateStr);
  };

  return (
    <div className="space-y-8 pb-12">
      <div className="flex justify-end mb-4">
        <div className="flex items-center gap-3 bg-white/50 dark:bg-gray-800/50 p-2 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
          <Button variant="ghost" size="icon" onClick={prevWeek}>
            <ChevronLeft size={20} />
          </Button>
          <span className="font-bold px-4 min-w-[150px] text-center text-xs uppercase tracking-widest">
            {startOfWeek.toLocaleDateString("it-IT", {
              month: "short",
              day: "numeric",
            })}{" "}
            -{" "}
